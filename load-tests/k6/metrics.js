...
export const endpointDuration = Object.freeze(
  Object.fromEntries(
    Object.entries(ENDPOINT_METRICS).map(([endpoint, name]) => [endpoint, new Trend(name)]),
  ),
);

function boundedSoakBucketCount(rawSeconds) {
  const seconds = Number(rawSeconds ?? '1800');
  if (!Number.isSafeInteger(seconds) || seconds < 1 || seconds > SOAK_MAX_SECONDS) {
...
function boundedMinute(minute, bucketCount) {
  if (!Number.isSafeInteger(minute) || minute < 0 || minute >= bucketCount) {
    throw new RangeError(`soak minute must be between 0 and ${bucketCount - 1}`);
  }
  return minute;
}
...
function measurementTags(runKind, minute) {
  ...
  const bounded = minute === undefined
    ? Math.min(
      bucketCount - 1,
      Math.floor((Date.now() - (soakMeasurementStartedAtMs ??= Date.now())) / (SOAK_MINUTE_SECONDS * 1000)),
    )
...
function acceptedStatus(response, acceptedStatuses) {
  return Array.isArray(acceptedStatuses) && acceptedStatuses.includes(response.status);
}
...
export function recordResponse({
  ...
  if (!METRIC_ENDPOINTS.has(endpoint)) throw new RangeError('unknown load-test endpoint metric');
...
export function recordCheck(passed, measuring, {
  runKind = __ENV.RUN_KIND ?? 'probe',
  minute,
} = {}) {
  if (!measuring) return;
  checkFailed.add(!passed, measurementTags(runKind ?? __ENV.RUN_KIND ?? 'probe', minute));
}

export function recordCompletedLogin(measuring, {
  runKind = __ENV.RUN_KIND ?? 'probe',
  minute,
} = {}) {
  if (!measuring) return;
  completedLoginFlows.add(1, measurementTags(runKind ?? __ENV.RUN_KIND ?? 'probe', minute));
}
...
function buildSoakSubmetricThresholds() {
  ...
  const entries = [];
  for (let minute = 0; minute < boundedSoakBucketCount(__ENV.SOAK_SECONDS); minute += 1) {
