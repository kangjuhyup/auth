...
test('parseOptions rejects non-integers and soak durations outside the bounded range', () => {
  assert.throws(
    () => parseOptions({ MAX_VUS: '1.5' }),
    /MAX_VUS must be a positive integer/,
  );
  assert.throws(
    () => parseOptions({ WARMUP_SECONDS: '0' }),
    /WARMUP_SECONDS must be a positive integer/,
  );
  assert.throws(
    () => parseOptions({ SOAK_SECONDS: '0' }),
    /SOAK_SECONDS must be between 1 and 1800 seconds/,
  );
  assert.throws(
    () => parseOptions({ SOAK_SECONDS: '1801' }),
    /SOAK_SECONDS must be between 1 and 1800 seconds/,
  );
  assert.throws(
    () => parseOptions({ SOAK_SECONDS: '1.1' }),
    /SOAK_SECONDS must be between 1 and 1800 seconds/,
  );
  assert.throws(
    () => parseOptions({ LOAD_TEST_MODE: 'soak' }),
    /LOAD_TEST_MODE must be one of: capacity, smoke/,
  );
});
...
test('createRuntimeEnvironment rejects entropy values with a wrong requested length', () => {
  assert.throws(
    () =>
      createRuntimeEnvironment(parseOptions({ MAX_VUS: '50' }), () =>
        Buffer.alloc(32, 7),
      ),
...
test('resolveLoadTestIdentity accepts only bounded nonnegative process IDs', () => {
  assert.deepEqual(
    resolveLoadTestIdentity(
      () => 12345,
      () => 23456,
  ...
  assert.deepEqual(resolveLoadTestIdentity(undefined, undefined), {});

  for (const invalid of [-1, 1.5, Number.MAX_SAFE_INTEGER, '123']) {
    assert.throws(
      () =>
        resolveLoadTestIdentity(
          () => invalid,
      ...
      /process UID must be a bounded nonnegative integer/,
    );
    assert.throws(
      () =>
        resolveLoadTestIdentity(
      ...
      /process GID must be a bounded nonnegative integer/,
    );
  }
});
