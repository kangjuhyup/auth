...
| 컬럼 | 타입 | 설명 |
|------|------|------|
| tenant_id | bigint (PK, FK) | tenant.id |
...
| secret_hash | varchar(255) | 클라이언트 시크릿 해시 (nullable) |
| name | varchar(128) | 클라이언트 이름 |
| type | varchar(20) | `confidential` \| `public` \| `service` (기본: public) |
| enabled | boolean | 활성화 여부 (기본: true) |
| redirect_uris | json | 허용 리다이렉트 URI 목록 |
...
| post_logout_redirect_uris | json | 로그아웃 후 리다이렉트 URI 목록 |
| created_at | timestamp | |
| updated_at | timestamp | |
...
| status | varchar(20) | `ACTIVE` \| `LOCKED` \| `DISABLED` (기본: ACTIVE) |
| created_at | timestamp | |
| updated_at | timestamp | |
...
| enabled | boolean | 활성화 여부 (기본: true) |
| expires_at | timestamp | 만료 시점 (nullable) |
| created_at | timestamp | |
| updated_at | timestamp | |
...
| consumed_at | timestamp | 소비 시점 (nullable) |
| expires_at | timestamp | 만료 시점 (nullable) |
| created_at | timestamp | |
...
| rotated_at | timestamp | 로테이션 시점 (nullable) |
| expires_at | timestamp | 만료 시점 (nullable) |
| created_at | timestamp | |
...
| granted_at | timestamp | 동의 시점 |
| revoked_at | timestamp | 철회 시점 (nullable) |
...
| id | bigint (PK, auto) | |
| tenant_id | bigint (FK) | tenant.id (CASCADE) |
| user_id | bigint (FK) | user.id (nullable, SET NULL) |
...
도메인 이벤트 저장소. 비즈니스 애그리거트의 상태 변화를 이벤트 단위로 기록한다.

| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | char(26) (PK) | ULID |
| aggregate_id | char(26) | 애그리거트 ID |
| version | int | 이벤트 순서 번호 (낙관적 동시성 제어) |
| event_type | varchar(128) | 이벤트 타입 |
| data | json | 이벤트 페이로드 |
| occurred_at | timestamp | 발생 시점 |

- UK: `uk_es_events_aggregate_version` (aggregate_id, version)
- IDX: (aggregate_id)

#### `projection_checkpoint`

프로젝션 적용 상태 추적. 멱등성 보장 및 중복 이벤트 방지에 사용한다.

| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | bigint (PK, auto) | |
| projector | varchar(128) | 프로젝터 이름 |
| aggregate_id | char(26) | 애그리거트 ID |
| last_version | int | 마지막 적용 버전 (기본: 0) |
| updated_at | timestamp | |

- UK: `uk_checkpoint_projector_aggregate` (projector, aggregate_id)
