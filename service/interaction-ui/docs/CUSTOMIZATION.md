...
OIDC authorize 흐름 중 **로그인·동의·MFA**를 담당하는 SPA입니다. 소스는 [`service/interaction-ui/`](../) 워크스페이스(`@auth/interaction-ui`)에 있으며, 빌드 산출물은 Nest가 같은 오리진에서 서빙합니다.
...
| 구성요소                                                                                             | 역할                                                                          |
| ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| [vite.config.ts](../vite.config.ts)                                                                  | `base: '/interaction-assets/'` — JS/CSS는 이 prefix 아래로 빌드됨             |
...
- API 클라이언트 [`api/client.ts`](../src/api/client.ts)는 `apiBase()`를 **`window.location.pathname`** 으로 두어, 같은 경로 아래의 상대 URL로 요청합니다.
...
- 정적 자산 요청은 **`/interaction-assets/assets/...`** 입니다. `base`를 바꾸면 [index.html](../index.html) 생성 규칙과 Nest `serveRoot`를 **함께** 맞춰야 합니다.
...
- [`index.css`](../src/index.css) — 레이아웃, `.card`, `.btn`, `.btn-primary`, 폼 요소 등. **브랜드 색·타이포·다크 모드**는 여기서 조정하는 것이 가장 단순합니다.
- [`main.tsx`](../src/main.tsx)에서 CSS import 순서만 바꾸지 않도록 주의합니다.

### 4.2 페이지·컴포넌트

| 경로                                                                  | 역할                                                             |
| --------------------------------------------------------------------- | ---------------------------------------------------------------- |
| [App.tsx](../src/App.tsx)                                             | `getDetails()` 후 `login` / `consent` / `error` 라우팅, MFA 진입 |
| [pages/LoginPage.tsx](../src/pages/LoginPage.tsx)                     | ID/PW 폼, `idpList` 기반 IdP 버튼                                |
| [pages/ConsentPage.tsx](../src/pages/ConsentPage.tsx)                 | 스코프 동의                                                      |
| [pages/MfaPage.tsx](../src/pages/MfaPage.tsx) + `components/Mfa*.tsx` | MFA UI                                                           |
| [components/IdpButton.tsx](../src/components/IdpButton.tsx)           | 소셜 버튼 표시                                                   |
...
- TypeScript 검사 + Vite 빌드가 [`package.json`](../package.json) 스크립트에 정의되어 있습니다.
...
1. [vite.config.ts](../vite.config.ts)의 `base`
...
- 프로토콜·interaction 세션 개요: [OIDC.md](../../docs/OIDC.md) §4.6
- OIDC 공개 엔드포인트 명세: service `/openapi.json` 또는 Auth Docs Redoc

핵심 파일:

- [interaction.controller.ts](../../src/presentation/controllers/interaction.controller.ts)
- [interaction-ui/src/api/client.ts](../src/api/client.ts)
