...
export class Migration20260831000000 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(`
      DECLARE @oidc_model_pk NVARCHAR(128);
      SELECT @oidc_model_pk = kc.[name]
      FROM sys.key_constraints kc
      WHERE kc.[type] = 'PK'
        AND kc.[parent_object_id] = OBJECT_ID(N'dbo.[oidc_model]');
      IF @oidc_model_pk IS NOT NULL
        EXEC(N'ALTER TABLE [oidc_model] DROP CONSTRAINT [' + REPLACE(@oidc_model_pk, ']', ']]') + N']');
    `);
    this.addSql(
      'ALTER TABLE [oidc_model] ADD [tenant_id] NVARCHAR(64) NOT NULL;',
    );
    this.addSql(
